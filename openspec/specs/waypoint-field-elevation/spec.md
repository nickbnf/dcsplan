# Spec: Waypoint Field Elevation

## Purpose

The **waypoint field elevation** capability adds an optional `groundAlt` field (feet MSL) to the first and last waypoints of a flight plan, representing takeoff and landing field elevations respectively. The takeoff field elevation is used by the `performance-regime` capability to anchor leg-0 climb computation to the actual departure altitude rather than assuming sea level. The landing field elevation is stored for future use.
