...
Map objects are plan-level overlays drawn on the planner map and kneeboard pages. They come in two kinds: **library refs** (references to entries in the theatre library) and **plan-local markers**. Both are rendered using pictograms from the closed catalog and may carry threat range rings. This spec covers the data model, UI interaction, map rendering, and kneeboard rendering for map objects.
