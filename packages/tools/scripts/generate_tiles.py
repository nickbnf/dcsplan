...
def generate_tiles(image_path, output_dir, zoom_levels=None, tile_size=256):
    ...
    # "Natural" zoom level is the zoom level where pixel are one to one with the original map
    ...
    for zoom in zoom_levels:
        ...
        print(
            f"tile_size_in_original_image: {tile_size_in_original_image} -> {tile_size}",
            file=sys.stderr,
        )
            ...
            for y in range(nb_tiles_h):
                ...
                if tile.size != (tile_size, tile_size):
                    if zoom == natural_zoom:
                        # At natural zoom, pad with black instead of resizing to preserve pixel fidelity
...
Examples:
  ...
  python generate_tiles.py map.png --max-zoom 6
    ...
    success = generate_tiles(
        args.input_image, args.output_directory, zoom_levels, args.tile_size
    )
