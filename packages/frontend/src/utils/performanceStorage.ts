...
export const PERFORMANCE_STORAGE_KEY = 'dcsplan.performance';

export type AircraftParseResult =
...
/** Validate a raw aircraft block (not the versioned file wrapper). */
export function parseAircraftBlock(ac: unknown): AircraftParseResult {
  const errors: string[] = [];

  if (typeof ac !== 'object' || ac === null || Array.isArray(ac)) {
    return { ok: false, errors: ['Missing or invalid aircraft block.'] };
  }

  const a = ac as Record<string, unknown>;

  if (typeof a['model'] !== 'string') errors.push('"model" must be a string.');
  if (typeof a['takeoffConfiguration'] !== 'string') errors.push('"takeoffConfiguration" must be a string.');
  if (!isNonNegNum(a['taxiFuel'])) errors.push('"taxiFuel" must be a number >= 0.');

  const to = a['takeoff'];
  if (typeof to !== 'object' || to === null || Array.isArray(to)) {
    errors.push('"takeoff" must be an object.');
  } else {
    const t = to as Record<string, unknown>;
    if (!isNonNegNum(t['timeSec'])) errors.push('"takeoff.timeSec" must be a number >= 0.');
    if (!isNonNegNum(t['fuel'])) errors.push('"takeoff.fuel" must be a number >= 0.');
    if (!isNonNegNum(t['distance'])) errors.push('"takeoff.distance" must be a number >= 0.');
  ...
  if (!Array.isArray(a['regimes'])) {
    errors.push('"regimes" must be an array.');
  } else {
    ...
    regimes.forEach((r, i) => {
      ...
      if (typeof regime['id'] !== 'string' || !regime['id'])
        errors.push(`${prefix}: "id" must be a non-empty string.`);
      if (typeof regime['name'] !== 'string' || !regime['name']) {
        errors.push(`${prefix}: "name" must be a non-empty string.`);
      } else {
        const name = regime['name'] as string;
        if (seenNames.has(name)) errors.push(`${prefix}: duplicate regime name "${name}".`);
        seenNames.add(name);
      }
...
export function loadPerformance(): Aircraft {
  try {
    const raw = localStorage.getItem(PERFORMANCE_STORAGE_KEY);
    if (!raw) return defaultAircraft();
    const parsed = JSON.parse(raw);
    const result = parseAircraftBlock(parsed);
    return result.ok ? result.aircraft : defaultAircraft();
  } catch {
    return defaultAircraft();
  }
}

export function savePerformance(aircraft: Aircraft): void {
  try {
    localStorage.setItem(PERFORMANCE_STORAGE_KEY, JSON.stringify(aircraft));
  } catch (error) {
    if (error instanceof DOMException &&
        (error.name === 'QuotaExceededError' || error.name === 'SecurityError')) {
      console.warn('Storage not available, performance not persisted');
    }
  }
}
...
export function hasStoredPerformance(): boolean {
  try {
    return localStorage.getItem(PERFORMANCE_STORAGE_KEY) !== null;
  } catch {
    return false;
  }
}
